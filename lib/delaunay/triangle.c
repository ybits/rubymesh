...
int
triangle_equals(void *v1, void *v2) 
{
	Triangle *t1, *t2;
	t1 = v1;
	t2 = v2;
...
int
triangle_adjacent(Triangle *t1, Triangle *t2)
{
	int coincident_points = triangle_coincident_points(t1, t2);
...
int
triangle_coincident_point(Triangle *t1, Triangle *t2)
{
	int coincident_points = triangle_coincident_points(t1, t2);
...
double
triangle_circumcenter_x(Triangle *t)
{
	double exp = 2.0;
	return	
	(
...
double
triangle_circumcenter_y(Triangle *t)
{
	double exp = 2.0;
	return	
	(
...
double
triangle_circumcenter_denominator(Triangle *t)
{
	return 2.0 *
	(
...
double
triangle_circumcircle_radius(Triangle *t, Point *center)
{
...
void
triangle_free(Triangle *t)
{
	point_free(t->p1);
	point_free(t->p2);
	point_free(t->p3);
	point_free(t->centroid);
	if (t->circumcircle) circle_free(t->circumcircle);
	free(t);
}
